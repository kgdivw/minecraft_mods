...
execute if score praatIdx db_state matches 0 as @a at @s if entity @e[type=villager_v2,r=16] run tellraw @s {"rawtext":[{"text":"§a[Dorpeling] §fWat zie jij er goed uit vandaag!"}]}
execute if score praatIdx db_state matches 1 as @a at @s if entity @e[type=villager_v2,r=16] run tellraw @s {"rawtext":[{"text":"§a[Dorpeling] §fJij wordt vast de baas van het dorp!"}]}
execute if score praatIdx db_state matches 2 as @a[tag=db_pak] at @s if entity @e[type=villager_v2,r=16] run tellraw @s {"rawtext":[{"text":"§a[Dorpeling] §fWauw, wat een prachtig blauw pak!"}]}
execute if score praatIdx db_state matches 3 as @a[tag=db_auto] at @s if entity @e[type=villager_v2,r=16] run tellraw @s {"rawtext":[{"text":"§a[Dorpeling] §fWat een gave auto heb jij!"}]}
execute if score praatIdx db_state matches 4 as @a[tag=db_hero] at @s if entity @e[type=villager_v2,r=16] run tellraw @s {"rawtext":[{"text":"§a[Dorpeling] §fJij hebt Herobrine verslagen, onze held!"}]}
execute if score praatIdx db_state matches 5 as @a at @s if entity @e[type=villager_v2,r=16] run tellraw @s {"rawtext":[{"text":"§a[Dorpeling] §fFijn dat jij er bent!"}]}
